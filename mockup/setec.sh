...
# New function: Copy the active wallet address to the clipboard.
...
# New function: Save persistent settings on exit.
persist_settings() {
    cat > .env <<EOF
...
token_creator_menu() {
    ...
    # (The rest of the Token Creator submenu remains unchanged.)
    # ...
    ...
    while true; do
        ...
        case "$choice" in
            ...
            1)
                ...
                echo -e "${GREEN}Updating metadata for token mint: $token_mint${NC}"
                read -p "Enter new token name (leave blank to keep current): " NEW_NAME
                read -p "Enter new token symbol (leave blank to keep current): " NEW_SYMBOL
                read -p "Enter new token description (leave blank to keep current): " NEW_DESC
                read -p "Enter new token image URL (leave blank to keep current): " NEW_IMAGE_URL
                METADATA_JSON=$(jq -n \
                    --arg name "$NEW_NAME" \
                    --arg symbol "$NEW_SYMBOL" \
                    --arg desc "$NEW_DESC" \
                    --arg image "$NEW_IMAGE_URL" \
                    '{name: $name, symbol: $symbol, description: $desc, image: $image}')
                echo "$METADATA_JSON" > token_metadata.json
                echo -e "${GREEN}Updating metadata using spl-token CLI...${NC}"
                spl-token update-metadata "$token_mint" --metadata-file token_metadata.json
                echo -e "${GREEN}Metadata updated successfully for $token_mint.${NC}"
                pause
                ;;
            2)
                if [ -z "$selected_coin" ]; then
                ...
                echo -e "${GREEN}Transferring update authority to $new_auth for token mint: $token_mint${NC}"
                spl-token authorize "$token_mint" update "$new_auth" --fee-payer "$selected_wallet"
                pause
                ;;
            3)
                if [ -z "$selected_coin" ]; then
            ...
            5)
                ...
                token_mint="$selected_coin"
                echo -e "${GREEN}Renouncing update authority (making token immutable) for token mint: $token_mint${NC}"
