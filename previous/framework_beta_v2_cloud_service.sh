...
# =============================
# Deploy Setec Token Manager as a Systemd Service
# =============================
deploy_setec_service() {
    echo "Setting up Setec Token Manager as a systemd service..."

    cat <<EOL | sudo tee /etc/systemd/system/setec-token-manager.service
[Unit]
Description=Setec All In One Token Manager
After=network.target

[Service]
ExecStart=/bin/bash $HOME/setec_token_manager.sh
WorkingDirectory=$HOME
User=$USER
Restart=always
...
EOL
    ...
    sudo systemctl daemon-reload
    sudo systemctl enable setec-token-manager.service
    sudo systemctl start setec-token-manager.service

    echo "Setec Token Manager service installed and started successfully."
}

read -p "Would you like to run Setec Token Manager as a systemd service? (y/n): " SERVICE_CHOICE
if [[ "$SERVICE_CHOICE" == "y" ]]; then
    deploy_setec_service
fi

# =============================
# Deploy Setec Token Manager as a Cloud-Based Web Application
# =============================

deploy_cloud_application() {
    echo "Setting up Setec Token Manager as a cloud-based web application..."

    # Prompt user for domain or subdomain
    read -p "Enter your domain or subdomain (e.g., setec.yourdomain.com): " USER_DOMAIN

    # Install necessary tools
    sudo apt update
    sudo apt install -y nginx python3 python3-pip python3-venv mysql-server ufw certbot python3-certbot-nginx

    # Set up MySQL database
    echo "Configuring MySQL database..."
    sudo mysql -e "CREATE DATABASE setec_db;"
    sudo mysql -e "CREATE USER 'setec_user'@'localhost' IDENTIFIED BY 'SetecSecurePass!';"
    sudo mysql -e "GRANT ALL PRIVILEGES ON setec_db.* TO 'setec_user'@'localhost';"
    sudo mysql -e "FLUSH PRIVILEGES;"

    # Create web application directory
    mkdir -p $HOME/setec_web_app
    cd $HOME/setec_web_app

    # Set up Python virtual environment
    python3 -m venv venv
    source venv/bin/activate
    pip install flask flask-mysql flask-login gunicorn

    # Create Flask app for user authentication
    cat <<EOL > app.py
...
app.config['MYSQL_USER'] = 'setec_user'
app.config['MYSQL_PASSWORD'] = 'SetecSecurePass!'
app.config['MYSQL_DB'] = 'setec_db'
...
@app.route('/')
def home():
    return "Welcome to Setec Token Manager"
...
EOL
    ...
    sudo tee /etc/nginx/sites-available/setec <<EOF
server {
    listen 80;
...
EOF
    ...
    sudo ln -s /etc/nginx/sites-available/setec /etc/nginx/sites-enabled/
    sudo systemctl restart nginx

    # Obtain SSL Certificate
    sudo certbot --nginx -d $USER_DOMAIN --non-interactive --agree-tos -m your-email@example.com

    echo "Setec Token Manager deployed at https://$USER_DOMAIN"
}

read -p "Would you like to deploy Setec Token Manager as a cloud-based service? (y/n): " CLOUD_SERVICE_CHOICE
if [[ "$CLOUD_SERVICE_CHOICE" == "y" ]]; then
    deploy_cloud_application
fi
